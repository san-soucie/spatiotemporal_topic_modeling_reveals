import pandas as pd
from ..utils import ConfigDict, FileList
...
@data_options
@pass_context
def process_data(ctx: Context, **kwargs) -> FileList:
    config = ctx.obj[ConfigDict.name]
    filelist = ctx.obj[FileList.name]

    classcount_path = config.raw_data_path / 'otz_classcount.csv'
    meta_data_path = config.raw_data_path / 'otz_meta_data.csv'
    sdgcorr_path = config.raw_data_path / 'SdGcorr.csv'
    classlist_path = config.raw_data_path / 'IFCB_classlist_type.csv'
    auxiliary_meta_data_path = config.raw_data_path / 'auxiliary_meta_data.json'

    filelist.classcount = config.processed_data_path / 'classcount.json'
    filelist.meta_data = config.processed_data_path / 'meta_data.json'
    filelist.data = config.processed_data_path / 'data.json'
    filelist.rost_input_observations = config.processed_data_path / 'observations.csv'
    ...
    if not kwargs['dry_run']:
        ...
        classcount.to_json(filelist.classcount, orient='records')
        ...
        good_classes_data = classcount[[c for c in classcount.columns if c not in bad_classes]]
        badmeta = ['lon', 'lat', 'depth']

        intermediate_meta_data = intermediate_meta_data[[c for c in intermediate_meta_data.columns if c not in badmeta]]
        ...
        processed_meta_data = processed_meta_data.drop(['sample_type', 'skip', 'dataset'], axis=1)
        processed_meta_data['sample_time'] = processed_meta_data.datetime.apply(pd.to_datetime).apply(lambda t: t.tz_localize(None))
        good_data.index = processed_meta_data.sample_time

        auxiliary_meta_data.sample_time = auxiliary_meta_data.sample_time.apply(pd.to_datetime)
        ...
        processed_meta_data.to_json(filelist.meta_data, orient='records')
        good_data.to_json(filelist.data, orient='records')

        observations = good_data.reset_index()
        observations = observations.drop(['sample_time'], axis=1)
        observations.to_csv(filelist.rost_input_observations, index=False)
    return filelist
